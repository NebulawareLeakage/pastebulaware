...
local function getSpeed()
	...
	if lplr.Character then 
		...
		if bedwarsStore.scythe ~= 0 then 
			local scythespeed = GuiLibrary.ObjectsThatCanBeSaved["SpeedScythe SpeedSlider"].Api.Value
...
runFunction(function()
	...
	local function autoLeaveAdded(plr)
		task.spawn(function()
			...
			local success, tagsfolder = pcall(function() return plr.Tags:IsA("Folder") and plr.Tags end)
			if success and tagsfolder then 
				for i,v in pairs(tagsfolder:GetChildren()) do 
					if v:IsA("StringValue") and v.Value:lower():find(" mod") then 
						lockdownConfiguration(plr)
					end
				end
			end
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					...
					if isAlive() and lplr.Character.Humanoid.FloorMaterial == Enum.Material.Air and bedwarsStore.scythe == 0 then 
